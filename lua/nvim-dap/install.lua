...
local function install(name, dir, script)
  ...
  local function on_exit(_, code)
    if code ~= 0 then
      vim.notify("Could not install " .. name .. "!", "error", { title = "dapinstall" })
    end

    vim.notify("Successfully installed " .. name .. "!", "success", { title = "dapinstall" })
  end
  ...
  if chan == 0 then
    vim.notify("Cannot create terminal instance!", "error", { title = "dapinstall" })
  end
